use std::{ops::{Add, Mul, Rem, Neg, Div}, borrow::Borrow};
...
impl<F: MontConfig<N>, const N: usize, Rhs> Add<Rhs> for PolyRing<F, N> where Rhs: Borrow<PolyRing<F, N>> {
...
impl<F: MontConfig<N>, const N: usize, Rhs> Mul<Rhs> for PolyRing<F, N> where Rhs: Borrow<PolyRing<F, N>> {
    type Output = PolyRing<F, N>;
...
impl<F: MontConfig<N>, const N: usize, Rhs> Mul<Rhs> for &PolyRing<F, N> where Rhs: Borrow<PolyRing<F, N>>  {
    ...
    fn mul(self, rhs: Rhs) -> Self::Output {
        PolyRing { poly: self.poly.naive_mul(&rhs.borrow().poly) }
    }
}
...
impl<F: MontConfig<N>, const N: usize, Rhs> Div<Rhs> for PolyRing<F, N> where Rhs: Borrow<PolyRing<F, N>> {
    type Output = PolyRing<F, N>;
...
impl<F: MontConfig<N>, const N: usize, const M: usize> From<[i64; M]> for PolyRing<F ,N> {
    fn from(data: [i64; M]) -> Self {
        Self {
            poly: DensePolynomial { coeffs: data.iter().map(|x| Fp::from(*x)).collect() }
        }
    }
}
