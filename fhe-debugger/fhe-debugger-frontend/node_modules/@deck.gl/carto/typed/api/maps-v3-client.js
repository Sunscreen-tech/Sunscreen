...
async function requestData({ method, url, accessToken, format, body }) {
    if (format === FORMATS.NDJSON) {
        return request({ method, url, accessToken, body });
    }
    const data = await requestJson({ method, url, accessToken, body });
 ...
 */
function getParameters({ type, source, geoColumn, columns, clientId, aggregationExp, aggregationResLevel }) {
...
export async function mapInstantiation({ type, source, connection, credentials, geoColumn, columns, clientId, aggregationExp, aggregationResLevel }) {
...
function checkFetchLayerDataParameters({ type, source, connection, credentials, geoColumn, columns, aggregationExp, aggregationResLevel }) {
...
export async function fetchLayerData({ type, source, connection, credentials, geoColumn, columns, format, formatTiles, clientId, aggregationExp, aggregationResLevel }) {
    ...
    const result = { data, format: mapFormat, schema: metadata.schema };
    return result;
}
async function _fetchDataUrl({ type, source, connection, credentials, geoColumn, columns, format, formatTiles, clientId, aggregationExp, aggregationResLevel }) {
    ...
    if (format === FORMATS.TILEJSON && formatTiles) {
        log.assert(Object.values(TILE_FORMATS).includes(formatTiles), `Invalid value for formatTiles: ${formatTiles}. Use value from TILE_FORMATS`);
        url += `&${encodeParameter('formatTiles', formatTiles)}`;
    }
    const { accessToken } = localCreds;
    return { url, accessToken, mapFormat, metadata };
}
export async function getData({ type, source, connection, credentials, geoColumn, columns, format, clientId }) {
...
export async function fetchMap({ cartoMapId, clientId, credentials, autoRefresh, onNewData }) {
    ...
    const map = await requestJson({ url, accessToken });
