...
function extractTextLayers(layers) {
    return flatten(layers.map(({ id, config, ...rest }) => {
...
function createChannelProps(visualChannels, type, config, data) {
    const { colorField, colorScale, sizeField, sizeScale, strokeColorField, strokeColorScale } = visualChannels;
    ...
    if (type === 'grid' || type === 'hexagon') {
        ...
        result.getFillColor = getColorAccessor(colorField, colorScale, visConfig.colorRange, visConfig.opacity, data);
    }
    if (strokeColorField) {
