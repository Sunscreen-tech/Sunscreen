...
export function tileIsEmpty(sd) {
...
export class TileMap {
    ...
    *getTilesOfLevel(z) {
    ...
    fillTheLowestLayer() {
        const tileMap = new IntPairMap(1);
    ...
    //   }
    // }
    regenerateCurveClipsUpToLayer(levelIndex, activeNodes) {
    ...
    filterOutEntities(levelToReduce, nodes, z) {
    ...
    subdivideLevel(z) {
        ...
        this.levels[z] = new IntPairMap(tilesInRow);
    ...
    subdivideTilesOnLevel(z) {
        ...
        for (const [key, tile] of this.levels[z - 1].keyValues()) {
            ...
            allTilesAreSmall && (allTilesAreSmall = tileIsSmall);
        }
        this.removeEmptyTiles(z);
    ...
    lowerTile, regenerate) {
        ...
        function intersectWithMiddleLines(seg) {
            // point, parameter
            const xs = Array.from(Curve.getAllIntersections(seg, horizontalMiddleLine, true)).concat(Array.from(Curve.getAllIntersections(seg, verticalMiddleLine, true)));
            xs.sort((a, b) => a.par0 - b.par0);
            const filteredXs = [[seg.start, seg.parStart]];
    ...
    generateSubTileExceptEdgeClips(upperTile, tileRect) {
...
function pushToClips(clips, e, c) {
    if (c instanceof Curve) {
        for (const seg of c.segs) {
