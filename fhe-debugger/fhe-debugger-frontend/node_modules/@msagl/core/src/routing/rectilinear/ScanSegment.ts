...
export class ScanSegment extends SegmentBase {
  ...
  CreateSparseVerticesAndEdges(vg: VisibilityGraph) {
    ...
    for (const perpCoord of Array.from(this.sparsePerpendicularCoords.values()).sort(compareNumbers)) {
      ...
      this.AppendVisibilityVertex(vg, vg.FindVertex(vertexLocation) ?? vg.AddVertexP(vertexLocation))
    }
  ...
  private AppendGroupCrossingsThroughPoint(vg: VisibilityGraph, lastPoint: Point): boolean {
    if (this.GroupBoundaryPointAndCrossingsList == null) {
      return false
    }
    ...
    while (this.GroupBoundaryPointAndCrossingsList.CurrentIsBeforeOrAt(lastPoint)) {
      ...
      found = true
      const crossingVertex = vg.FindVertex(pac.Location) ?? vg.AddVertexP(pac.Location)
  ...
  private static GetCrossingInteriorVertex(
    ...
    const interiorPoint: Point = crossing.GetInteriorVertexPoint(crossingVertex.point)
  ...
  private AddLowCrossings(vg: VisibilityGraph, crossingVertex: VisibilityVertex, crossings: GroupBoundaryCrossing[]) {
    if (crossings != null) {
  ...
  private AddHighCrossings(vg: VisibilityGraph, crossingVertex: VisibilityVertex, crossings: GroupBoundaryCrossing[]) {
    if (crossings != null) {
      const interiorVertex: VisibilityVertex = ScanSegment.GetCrossingInteriorVertex(vg, crossingVertex, crossings[0])
