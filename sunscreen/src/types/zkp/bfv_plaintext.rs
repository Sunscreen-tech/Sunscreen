...
/// A BFV plaintext polynomial with signed encoding that has been shared to a ZKP program.
///
...
/// A [BFV signed integer](Signed) that has been shared to a ZKP program.
///
...
pub struct BfvSigned<F: FieldSpec>(
    BfvSignedEncoding<F, { <Signed as ShareWithZkp>::DEGREE_BOUND }>,
);

/// A [BFV unsigned 64-bit integer](Unsigned64) that has been shared to a ZKP program.
///
/// Use the [`AsFieldElement::into_field_elem`] method to decode the value into a field
/// element within a ZKP program.
#[derive(Debug, Clone, TypeName)]
pub struct BfvUnsigned64<F: FieldSpec>(
    BfvSignedEncoding<F, { <Unsigned64 as ShareWithZkp>::DEGREE_BOUND }>,
);

/// A [BFV unsigned 128-bit integer](Unsigned128) that has been shared to a ZKP program.
///
/// Use the [`AsFieldElement::into_field_elem`] method to decode the value into a field
/// element within a ZKP program.
#[derive(Debug, Clone, TypeName)]
pub struct BfvUnsigned128<F: FieldSpec>(
    BfvSignedEncoding<F, { <Unsigned128 as ShareWithZkp>::DEGREE_BOUND }>,
);

/// A [BFV rational](crate::types::bfv::Rational) that has been shared to a ZKP program.
///
...
macro_rules! impl_shared_zkp_type {
    ($($int_type:ident),+) => {
        $(
            paste! {
                impl<F: FieldSpec> DynamicNumFieldElements for [<Bfv $int_type>]<F> {
                    fn num_native_field_elements(plaintext_modulus: u64) -> usize {
                        <BfvSignedEncoding<F, { <$int_type as ShareWithZkp>::DEGREE_BOUND }> as DynamicNumFieldElements>::
                ...
                impl<F: FieldSpec> AsFieldElement<F> for ProgramNode<[<Bfv $int_type>]<F>> {
                    ...
                    fn into_field_elem(self) -> Self::Output {
                        let cast: ProgramNode<BfvSignedEncoding<F, { <$int_type as ShareWithZkp>::DEGREE_BOUND }>> =
