...
use crate::{error::{Error, Result}, internals::attr::ZkpProgramAttrs};
...
pub fn zkp_program_impl(
    ...
    fn parse_inner(_attr_params: ZkpProgramAttrs, input_fn: ItemFn) -> Result<proc_macro::TokenStream> {
        ...
        match ret {
            ReturnType::Default => {}
            _ => {
                return Err(Error::compile_error(ret.span(), "ZKP programs may not return values."));
            }
        };
        ...
        let var_decl = unwrapped_inputs.iter().enumerate().map(|(i, t)| {
            ...
            create_program_node(&var_name, t.1, "private_input")
        });
