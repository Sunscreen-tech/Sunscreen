...
__device__ Scalar29 Scalar29::unpack(const u32* words, size_t grid_tid, size_t stride) {
...
__device__ void Scalar29::pack(u32* words, size_t grid_tid, size_t stride) {
    words = &words[grid_tid];
...
__device__ Scalar29 Scalar29::add(const Scalar29 a, const Scalar29 b) {
...
__device__ Scalar29 Scalar29::sub(const Scalar29 a, const Scalar29 b) {
...
struct MulResult {
    ...
    __device__ const u64& operator[](const size_t i) const {
        return limbs[i];
    }

    __device__ u64& operator[](const size_t i) {
        return limbs[i];
    }
};

__device__ MulResult square_internal(Scalar29 a);
__device__ MulResult mul_internal(Scalar29 a, Scalar29 b);
__device__ Scalar29 montgomery_reduce(MulResult limbs);

__device__ Scalar29 Scalar29::mul(Scalar29 a, Scalar29 b) {
...
__device__ Scalar29 Scalar29::square(Scalar29 a) {
...
__device__ inline u64 m(u32 a, u32 b) {
    return ((u64)a) * ((u64)b);
}
...
struct MontMulLRes {
        ...
        __device__ MontMulLRes(u64 carry, u32 n): carry(carry), n(n) {}
};

__device__ inline MontMulLRes part1(u64 sum) {
    u32 p = ((u32)sum) * (constants::LFACTOR) & ((1u << 29) - 1);
    return MontMulLRes((sum + m(p,constants::L()[0])) >> 29, p);
}

__device__ inline MontMulLRes part2(u64 sum) {
...
__device__ Radix16 Scalar29::as_radix_16() const {
    Radix16 output;
...
__device__ Scalar29 Scalar29::operator-() const {
    auto zero = Scalar29::Zero();
    return Scalar29::sub(zero, *this);
}

__device__ Scalar29 montgomery_square(const Scalar29& x) {
    return montgomery_reduce(square_internal(x));
}

__device__ Scalar29 montgomery_mul(const Scalar29& a, const Scalar29& b) {
    return montgomery_reduce(mul_internal(a, b));
}

__device__ void square_multiply(volatile Scalar29& y, size_t squarings, const Scalar29& x) {
    for (size_t i = 0; i < squarings; i++) {
        y = montgomery_square((Scalar29&)y);
    }
    y = montgomery_mul((Scalar29&)y, x);
}

__device__ Scalar29 Scalar29::montgomery_invert() const {
...
__device__ Scalar29 to_montgomery(const Scalar29& val) {
    auto rr = constants::RR();
    return montgomery_mul(val, rr);
}

__device__ Scalar29 from_montgomery(const Scalar29& val) {
...
__device__ Scalar29 Scalar29::invert() const {
