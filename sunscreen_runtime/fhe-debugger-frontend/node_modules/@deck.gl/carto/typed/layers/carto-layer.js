...
export default class CartoLayer extends CompositeLayer {
    ...
    _checkProps(props) {
        ...
        log.assert(Object.values(API_VERSIONS).includes(apiVersion), `Invalid apiVersion ${apiVersion}. Use API_VERSIONS enum.`);
        if (apiVersion === API_VERSIONS.V1 || apiVersion === API_VERSIONS.V2) {
            log.assert(type === MAP_TYPES.QUERY || type === MAP_TYPES.TILESET, `Invalid type ${type}. Use type MAP_TYPES.QUERY or MAP_TYPES.TILESET for apiVersion ${apiVersion}`);
            ...
            log.assert(Object.values(MAP_TYPES).includes(type), `Invalid type ${type}. Use MAP_TYPES enum.`);
            ...
            if (!GEO_COLUMN_SUPPORT.includes(type)) {
                log.assert(!geoColumn, `geoColumn prop is only supported for types: ${GEO_COLUMN_SUPPORT.join()}`);
            }
            if (columns) {
    ...
    async _updateData() {
        try {
            ...
            if (apiVersion === API_VERSIONS.V1 || apiVersion === API_VERSIONS.V2) {
                result = {
                    data: await getDataV2({ type, source, credentials: credentials })
                };
            }
            else {
    ...
    _getSubLayerAndProps() {
        ...
        if (format === FORMATS.TILEJSON) {
            ...
            props.formatTiles =
                props.formatTiles ||
                    tileUrl.searchParams.get('formatTiles') ||
                    TILE_FORMATS.MVT;
            if (data.scheme === 'h3') {
