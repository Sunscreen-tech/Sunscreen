...
export function getDeckInstance({ map, gl, deck }) {
    ...
    deck.userData.mapboxLayers = new Set();
    deck.userData.mapboxVersion = getMapboxVersion(map);
    map.__deck = deck;
    map.on('render', () => {
...
function getViewport(deck, map, useMapboxProjection = true) {
    const { mapboxVersion } = deck.userData;
    return new WebMercatorViewport(Object.assign({
        ...
        ? {
            ...
            nearZMultiplier: (mapboxVersion.major === 1 && mapboxVersion.minor >= 3) || mapboxVersion.major >= 2
                ? 0.02
                : 1 / (deck.height || 1)
        }
        : {
