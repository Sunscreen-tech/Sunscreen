...
export class SegmentIntersector {
  ...
  constructor() {
    this.verticalSegmentsScanLine = new RBTree<ScanSegment>((a, b) => this.CompareSS(a, b))
  ...
  RemoveSegmentsWithNoVisibility(horizontalScanSegments: ScanSegmentTree, verticalScanSegments: ScanSegmentTree) {
    for (const seg of this.segmentsWithoutVisibility) {
      const segs = seg.IsVertical ? verticalScanSegments : horizontalScanSegments
      segs.Remove(seg)
    }
  }
  ...
  ScanIntersect(hSeg: ScanSegment) {
    ...
    for (; null != segNode; segNode = this.verticalSegmentsScanLine.next(segNode)) {
      ...
      const newVertex: VisibilityVertex = this.visGraph.AddVertexP(new Point(vSeg.Start.x, hSeg.Start.y))
